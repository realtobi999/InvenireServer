...
public class CreatePropertySuggestionCommandHandlerTests
{
    ...
    public async Task Handle_ReturnsCorrectInstance()
    {
        ...
        _services.Setup(s => s.Organizations.GetForAsync(employee)).ReturnsAsync(organization);
        _services.Setup(s => s.Properties.GetForAsync(organization)).ReturnsAsync(property);
    ...
    public async Task Handle_ThrowsExceptionWhenEmployeeDoesntBelongInOrganization()
    {
        ...
        _services.Setup(s => s.Employees.GetAsync(command.Jwt)).ReturnsAsync(employee);
        _services.Setup(s => s.Organizations.GetForAsync(employee)).ReturnsAsync((Organization?)null);
    ...
    public async Task Handle_ThrowsExceptionWhenPropertyIsNotCreated()
    {
        ...
        _services.Setup(s => s.Employees.GetAsync(command.Jwt)).ReturnsAsync(employee);
        _services.Setup(s => s.Organizations.GetForAsync(employee)).ReturnsAsync(organization);
        _services.Setup(s => s.Properties.GetForAsync(organization)).ReturnsAsync((Property?)null);
