...
public class SameAsAttribute : ValidationAttribute
{
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext context)
    {
        ...
        if (property == null) return new ValidationResult($"Property '{_cProperty}' not found.");

        var cValue = property.GetValue(context.ObjectInstance);

        // Compare the values.
        if (!Equals(value, cValue)) return new ValidationResult($"The {context.DisplayName} and {_cProperty} fields do not match.");
