...
public class ExportToExcelPropertyItemQueryHandler : IRequestHandler<ExportToExcelPropertyItemQuery, Stream>
{
            ...
            foreach (var item in items)
            {
                if (item.EmployeeId is not null)
                {
                    ...
                    else
                    {
                        ...
                        }) ?? throw new NotFound404Exception("The employee was not found");
                    }
                }
    ...
    private static void SetHeaders(IXLWorksheet worksheet)
    {
        ...
        worksheet.Cell(1, 11).Value = "Employee:Name";
        worksheet.Cell(1, 12).Value = "Employee:Email";
        worksheet.Cell(1, 13).Value = "Description";
        worksheet.Cell(1, 14).Value = "Document-Number";
    }
    ...
    private static void SetRow(IXLWorksheet worksheet, PropertyItemDto item, int row)
    {
        ...
        worksheet.Cell(row, 11).Value = item.Employee?.FullName ?? "";
        worksheet.Cell(row, 12).Value = item.Employee?.EmailAddress ?? "";
        worksheet.Cell(row, 13).Value = item.Description ?? "";
        worksheet.Cell(row, 14).Value = item.DocumentNumber ?? "";
    }
